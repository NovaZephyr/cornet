...
function Logo() { return <Link to="/" className="flex items-center gap-2"><img src={logoMark} alt="Logo de CoreNetwork" width={512} height={512} className="h-8 w-8 rounded-lg" /><span className="text-xl font-bold tracking-tighter">CoreNetwork</span></Link>; }

function SiteFooter() { const links: { to: "/about" | "/blog" | "/rules" | "/partner" | "/community" | "/playlists" | "/settings"; label: string }[] = [{ to: "/about", label: "About" }, { to: "/blog", label: "Press & Blogs" }, { to: "/about", label: "Copyright" }, { to: "/partner", label: "Creators & Partners" }, { to: "/about", label: "Advertising" }, { to: "/playlists", label: "Playlists" }, { to: "/about", label: "Terms" }, { to: "/about", label: "Privacy" }, { to: "/about", label: "Report a bug" }, { to: "/settings", label: "Try something new" }]; return <footer className="cn-2012-footer border-t border-border px-6 py-8 pb-24 md:pb-8"><div className="mx-auto flex max-w-5xl flex-col items-center gap-3 text-center"><div className="flex items-center gap-2"><img src={logoMark} alt="" className="h-6 w-6 opacity-70" /><span className="text-sm font-bold tracking-tighter">CoreNetwork</span></div><nav className="flex max-w-3xl flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[11px] text-muted-foreground">{links.map((l) => <Link key={`${l.to}-${l.label}`} to={l.to} className="transition-colors hover:text-foreground hover:underline">{l.label}</Link>)}</nav><div className="flex flex-wrap justify-center gap-2 text-[11px]"><span className="rounded border border-border bg-surface px-2 py-1">Español</span><span className="rounded border border-border bg-surface px-2 py-1">Worldwide</span><span className="rounded border border-border bg-surface px-2 py-1">Safety: On</span></div><p className="text-[11px] text-muted-foreground">© {new Date().getFullYear()} CoreNetwork.</p></div></footer>; }
...
export function AppShell({ children, hideSidebar = false }: { children: ReactNode; hideSidebar?: boolean }) {
  ...
  return (
    <div className="cn-2012-app min-h-screen bg-background">
      <SiteBanner />
      <header className="cn-2012-topbar sticky top-0 z-50 flex h-14 items-center gap-2 bg-background px-2 sm:px-4">
        ...
        <form className="mx-auto flex w-full max-w-xl items-center px-2" onSubmit={(e) => { e.preventDefault(); void navigate({ to: "/", search: query ? { q: query } : {} }); }}>
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar videos y canales" className="h-10 rounded-l-full rounded-r-none border-border bg-input px-4 focus-visible:ring-0" />
