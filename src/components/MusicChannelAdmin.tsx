...
export function MusicChannelAdmin() {
  ...
  return <section className="rounded-2xl border border-border bg-surface p-4"><div className="mb-3 flex items-center gap-2"><Music2 className="h-5 w-5 text-primary" /><div><h2 className="font-semibold">Canales de música</h2><p className="text-xs text-muted-foreground">Marca canales para mostrar la insignia musical junto a sus demás distinciones.</p></div></div><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar canal por @usuario o nombre" className="pl-9" /></div><div className="mt-3 space-y-2">{query.trim().length >= 2 && channels.data?.map((channel) => <div key={channel.id} className="flex items-center gap-3 rounded-xl border border-border/70 bg-background/60 p-3"><ChannelAvatar path={channel.avatar_path} name={channel.display_name || channel.username} size={42} /><div className="min-w-0 flex-1"><p className="truncate font-medium">{channel.display_name || channel.username}</p><p className="truncate text-xs text-muted-foreground">@{channel.username}</p></div><Switch checked={channel.is_music_channel === true} onCheckedChange={(value) => void toggle(channel.id, value)} aria-label={`Canal de música de @${channel.username}`} /><span title="Canal de música" aria-hidden="true" className="inline-flex h-8 w-8 items-center justify-center text-primary"><Music2 className="h-4 w-4" /></span></div>)}{query.trim().length >= 2 && !channels.isLoading && !channels.data?.length && <p className="py-5 text-center text-sm text-muted-foreground">No encontramos canales.</p>}</div></section>;
}
