...
function safeName(name: string) {
  return name.replace(/[\\/\0]/g, "-").trim().slice(0, 255) || "archivo";
}
...
async function signedB2Link(file: StoredFile | TempFile, temporary?: boolean) {
  const remaining = temporary
    ? Math.max(60, Math.floor((new Date((file as TempFile).expires_at).getTime() - Date.now()) / 1000))
    : 3600;
  if (temporary && remaining <= 0) throw new Error("El archivo temporal ya expiró.");
  const expiresIn = Math.min(remaining, 3600);
  const result = await createB2DownloadUrl(file.storage_path, safeName(file.original_name), expiresIn);
  return result.url;
}
...
export function CommunityStorage() {
  ...
  const tempQuery = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const { data, error } = await db.from("community_temp_files").select("id,user_id,original_name,storage_path,mime_type,size_bytes,created_at,expires_at").eq("user_id", user!.id).order("created_at", { ascending: false });
      if (error) throw error;
  ...
  const uploadTemporary = async (file: File) => {
    ...
    const storedName = safeName(file.name);
    const key = `temp/${user.id}/${crypto.randomUUID()}-${storedName}`;
    try {
      await uploadToB2(key, file);
      const { error: dbError } = await db.from("community_temp_files").insert({ user_id: user.id, original_name: storedName, storage_path: key, mime_type: file.type || null, size_bytes: file.size, expires_at: expiresAt.toISOString() });
      if (dbError) {
        await deleteFromB2(key).catch(() => undefined);
        throw dbError;
      }
      const result = await createB2DownloadUrl(key, storedName, Math.min(tempDuration, 3600));
      setLastTempUrl(result.url);
      await qc.invalidateQueries({ queryKey: ["community-temp-files", user.id] });
      toast.success("Enlace temporal creado en Backblaze B2");
    } catch (error) {
