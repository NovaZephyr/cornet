...
async function notifyMentions(content: string, link: string, body: string) {
  if (!content.trim()) return;
  const { error } = await supabase.rpc("notify_mentions", {
    _content: content,
    _link: link,
    _notification_type: "mention",
    _title: "Te mencionaron en la comunidad",
    _body: body,
  });
  if (error) console.error("[Community] mention notification failed", error);
}
...
export function CommunityFeed({ channelId }: { channelId?: string | undefined }) {
  ...
  const publish = async (e: React.FormEvent) => { e.preventDefault(); if (!user) return void toast.error("Inicia sesión para publicar"); if (!content.trim()) return; setBusy(true); try { let imagePath: string | null = null; if (image) imagePath = await uploadFile("media", user.id, image, "post-"); const { data: post, error } = await supabase.from("community_posts").insert({ user_id: user.id, content: content.trim(), image_path: imagePath }).select("id").single(); if (error) throw error; try { await syncCommunityPostHashtags(post.id, content.trim()); } catch (error) { console.error("[Community] Hashtag sync failed", error); } void notifyMentions(content.trim(), `/community#post-${post.id}`, `${profile?.display_name || profile?.username || "Alguien"} te mencionó en una publicación de comunidad.`); setContent(""); setImage(null); void qc.invalidateQueries({ queryKey: key }); toast.success("Publicado en la comunidad"); } catch (err) { toast.error(err instanceof Error ? err.message : "No se pudo publicar"); } finally { setBusy(false); } };
