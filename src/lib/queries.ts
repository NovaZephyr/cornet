...
export async function fetchProfilesByIds(ids: string[]): Promise<Map<string, ProfileLite>> {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id, username, display_name, avatar_path, is_verified, subscriber_count, channel_style")
    .in("id", unique);
...
export async function searchChannels(search?: string, orderBy: "subscribers" | "recent" = "subscribers") {
  let q = supabase
    .from("profiles")
    .select("id, username, display_name, avatar_path, is_verified, subscriber_count, channel_style, created_at")
    .limit(40);
  if (search?.trim()) {
    const escaped = search.trim().replace(/[%_]/g, "\\$&");
    q = q.or(`username.ilike.%${escaped}%,display_name.ilike.%${escaped}%`);
  }
  q = orderBy === "subscribers"
    ? q.order("subscriber_count", { ascending: false }).order("created_at", { ascending: false })
