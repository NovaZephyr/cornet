...
export async function fetchProfilesByIds(ids: string[]): Promise<Map<string, ProfileLite>> {
  ...
  if (error) {
    // The UI remains usable while a newly-created migration is still being applied.
...
export async function searchChannels(search?: string, orderBy: "subscribers" | "recent" = "subscribers") {
  ...
  let { data, error } = await supabase
    .from("profiles")
    .select(`${EXTENDED_PROFILE_FIELDS}, created_at`)
    .limit(40);
  ...
  if (orderBy === "subscribers") {
    profiles.sort((a, b) => (Number(b.subscriber_count ?? 0) - Number(a.subscriber_count ?? 0)) || (b.created_at.localeCompare(a.created_at)));
  } else {
...
export async function fetchVideos(options?: {
  ...
  let q = supabase
    .from("videos")
    .select("id, code, user_id, title, thumbnail_path, duration_seconds, views, created_at, category")
    .limit(limit);
  ...
  // New category column may not exist until the latest Supabase migration is applied.
  if (error) {
    ...
    else fallback = fallback.order("created_at", { ascending: orderBy === "oldest" });
    const result = await fallback;
    data = result.data;
    error = result.error;
  }
