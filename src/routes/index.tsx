...
export const Route = createFileRoute("/")({
  ...
  head: () => ({ meta: [{ title: "CoreNetwork — Free Yourself" }] }), component: Home,
});
...
function Home() {
  ...
  return <AppShell><h1 className="sr-only">Videos y canales en CoreNetwork</h1>
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div>{q ? <p className="text-sm text-muted-foreground">Resultados para <span className="font-medium text-foreground">“{q}”</span></p> : <p className="text-sm text-muted-foreground">Videos recomendados para ti</p>}</div><Select value={sort} onValueChange={(value) => setSort(value as SearchOrder)}><SelectTrigger className="w-48"><SelectValue placeholder="Ordenar" /></SelectTrigger><SelectContent><SelectItem value="subscribers">Más suscripciones</SelectItem><SelectItem value="views">Más vistos</SelectItem><SelectItem value="recent">Más recientes</SelectItem><SelectItem value="oldest">Más antiguos</SelectItem></SelectContent></Select></div>
