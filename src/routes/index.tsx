...
function Home() {
  ...
  const setSort = (next: SearchOrder) => {
    void navigate({
      to: "/",
      search: q ? { q, sort: next } : next === "recommended" ? {} : { sort: next },
    });
  };
      ...
      {isPolymerHome ? (
        <>
          ...
          <div className="cn-polymer-home-frame">
            {videosQuery.isLoading ? (
              ...
              polymerShelves.map(([title, videos], shelfIndex) => (
                <section className="cn-polymer-shelf" key={`${title}-${shelfIndex}`}>
                  <div className="cn-polymer-shelf-heading">
                    <h2>{title}</h2>
                    <span>{sort === "recommended" ? "Recommended videos" : "Videos"}</span>
                  </div>
        ...
        <>
                ...
                {channelsQuery.data.map((channel) => (
                  <Link key={channel.id} to="/c/$username" params={{ username: channel.username }} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3 transition-colors hover:bg-surface-hover">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-muted-foreground">@{channel.username} · {channel.subscriber_count ?? 0} suscriptores</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {videosQuery.isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {videosQuery.data.map((v) => <VideoCard key={v.id} video={v} />)}
            </div>
          ) : (
