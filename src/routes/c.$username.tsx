    ...
    return (
      <div className="cn-cosmic-channel" style={channelBackground}>
        <div className="cn-cosmic-channel-inner">
          <div className="cn-cosmic-topbar"><span>CoreNetwork</span><span className="cn-cosmic-topbar-right">/{profile.username}</span></div>
          <div className="cn-cosmic-banner-wrap">
            ...
            <div className="cn-cosmic-title-strip">
              <div className="flex min-w-0 items-center gap-3">
                <div className="relative"><ChannelAvatar path={profile.avatar_path} name={profile.display_name || profile.username} size={56} />{isPartnerChannel && partnerGif && <img src={partnerGif} alt="" className="absolute -bottom-1 -right-1 h-5 w-5 rounded-full border object-cover" />}</div>
                <div className="min-w-0"><h1 className="truncate text-[20px] font-bold">{profile.display_name || profile.username}{profile.is_verified && <VerifiedBadge className="ml-1 inline h-4 w-4" />}</h1><p className="text-[11px] opacity-70">@{profile.username} · {profile.subscriber_count ?? subs.length} suscriptores · {videos.length} videos</p></div>
              </div>
          ...
          <div className="cn-cosmic-columns">
            <aside className="cn-cosmic-left">
              <section className="cn-cosmic-panel"><h2>Información</h2>{infoLayout !== "hidden" ? <p>{profile.description || "Este canal todavía no tiene descripción."}</p> : <p className="opacity-60">Información oculta por el creador.</p>}<dl className="cn-cosmic-stats"><div><dt>Suscriptores</dt><dd>{profile.subscriber_count ?? subs.length}</dd></div><div><dt>Videos</dt><dd>{videos.length}</dd></div><div><dt>En CoreNetwork</dt><dd>{new Date(profile.created_at).toLocaleDateString("es")}</dd></div></dl></section>
              {(roles?.length ?? 0) > 0 && <section className="cn-cosmic-panel"><h2>Distinciones</h2><div className="flex flex-wrap gap-2">{roles?.includes("partner") && <Badge>Partner</Badge>}{roles?.includes("admin") && <Badge variant="secondary">Administrador</Badge>}{roles?.includes("moderator") && <Badge variant="secondary">Moderador</Badge>}</div></section>}
            ...
            <aside className="cn-cosmic-right">
              <section className="cn-cosmic-panel"><h2>Channel</h2><div className="cn-cosmic-profile-mini"><ChannelAvatar path={profile.avatar_path} name={profile.display_name || profile.username} size={52} /><div><strong>{profile.display_name || profile.username}</strong><span>@{profile.username}</span></div></div><Button className="mt-3 w-full cn-2012-retro-button" onClick={onSubscribe} variant={isSubscribed ? "secondary" : "default"}>{isSubscribed ? "Suscrito" : "Suscribirse"}</Button></section>
