...
import { BarChart3, FileText, Image as ImageIcon, Loader2, Plus, Save, ShieldCheck, Trash2 } from "lucide-react";
...
function AdminBlogPage() {
  ...
  useEffect(() => {
    if (isAdmin && user) void loadPosts();
  }, [isAdmin, user]);
  ...
  return <AppShell><div className="mx-auto max-w-5xl space-y-6 pb-16"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">Administración</p><h1 className="mt-2 text-3xl font-bold tracking-tight">Blog / Anuncios</h1><p className="mt-2 text-sm text-muted-foreground">Publicaciones oficiales de Cornet. Este sistema es editorial y está separado de campañas y publicidad.</p></div>
    <section className="rounded-2xl border border-border bg-surface p-6 shadow-sm"><div className="mb-5 flex items-center gap-2"><Plus className="h-5 w-5 text-primary"/><h2 className="font-semibold">Nueva publicación</h2></div><Tabs value={editorType} onValueChange={(value) => setEditorType(value as PostType)}><TabsList><TabsTrigger value="text"><FileText className="mr-1.5 h-4 w-4"/>Texto</TabsTrigger><TabsTrigger value="image"><ImageIcon className="mr-1.5 h-4 w-4"/>Imagen</TabsTrigger><TabsTrigger value="poll"><BarChart3 className="mr-1.5 h-4 w-4"/>Poll</TabsTrigger></TabsList><TabsContent value="text" className="space-y-4 pt-5"/><TabsContent value="image" className="space-y-4 pt-5"/><TabsContent value="poll" className="space-y-4 pt-5"/></Tabs><div className="mt-4 space-y-4"><div className="space-y-2"><Label>Título</Label><Input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} placeholder="Cornet se ha actualizado!"/></div><div className="space-y-2"><Label>{editorType === "poll" ? "Pregunta" : "Texto"}</Label><Textarea value={body} onChange={(event) => setBody(event.target.value)} rows={6} maxLength={5000} placeholder={editorType === "poll" ? "¿Qué función te gustaría ver después?" : "Escribe el anuncio oficial…"}/></div>{editorType === "image" && <div className="space-y-2"><Label>Ruta de imagen</Label><Input value={imagePath} onChange={(event) => setImagePath(event.target.value)} placeholder="media/anuncios/imagen.png o URL pública"/><p className="text-xs text-muted-foreground">Puedes usar una ruta del bucket media o una URL pública.</p></div>}{editorType === "poll" && <div className="space-y-2"><Label>Opciones</Label>{pollOptions.map((option, index) => <div key={index} className="flex gap-2"><Input value={option} onChange={(event) => setPollOptions((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} placeholder={`Opción ${index + 1}`}/>{pollOptions.length > 2 && <Button type="button" variant="ghost" size="icon" onClick={() => setPollOptions((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 className="h-4 w-4"/></Button>}</div>)}<Button type="button" variant="outline" onClick={() => setPollOptions((current) => [...current, ""])}><Plus className="mr-2 h-4 w-4"/>Añadir opción</Button></div>}<div className="flex flex-wrap gap-3"><Button onClick={() => void submit()} disabled={busy}><Save className="mr-2 h-4 w-4"/>{busy ? "Publicando…" : "Publicar"}</Button><Button variant="outline" onClick={resetEditor}>Limpiar</Button></div></div></section>
    <section className="rounded-2xl border border-border bg-surface p-6 shadow-sm"><div className="mb-5 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary"/><h2 className="font-semibold">Publicaciones recientes</h2></div><div className="space-y-2">{posts.map((post) => <article key={post.id} className="flex items-start gap-4 rounded-xl border border-border bg-background p-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">{post.post_type === "image" ? <ImageIcon className="h-5 w-5"/> : post.post_type === "poll" ? <BarChart3 className="h-5 w-5"/> : <FileText className="h-5 w-5"/>}</div><div className="min-w-0 flex-1"><strong className="block truncate">{post.title}</strong><p className="mt-1 text-xs text-muted-foreground">{post.post_type === "poll" ? "Poll" : post.post_type === "image" ? "Imagen" : "Texto"} · {timeAgo(post.created_at)}</p></div><Button variant="ghost" size="icon" onClick={() => void remove(post.id)} aria-label="Eliminar publicación"><Trash2 className="h-4 w-4"/></Button></article>)}{posts.length === 0 && <p className="py-10 text-center text-sm text-muted-foreground">Todavía no hay publicaciones.</p>}</div></section>
