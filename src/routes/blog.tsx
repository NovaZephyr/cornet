...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      { title: "Anuncios — CoreNetwork" },
      { name: "description", content: "Novedades y anuncios oficiales del equipo de CoreNetwork." },
      { property: "og:title", content: "Anuncios — CoreNetwork" },
    ],
  }),
  component: BlogPage,
});
...
function PollBlock({ announcementId }: { announcementId: string }) {
  ...
  const { data: options } = useQuery({
    queryKey: ["poll-options", announcementId],
    queryFn: async () => {
  ...
  const { data: results } = useQuery({
    queryKey: ["poll-results", announcementId],
    queryFn: async () => {
  ...
  const { data: myVote } = useQuery({
    queryKey: ["my-poll-vote", announcementId, user?.id],
    enabled: !!user,
    queryFn: async () => {
  ...
  const vote = async (optionId: string) => {
    ...
    const { error } = await supabase
      ...
      .insert({ announcement_id: announcementId, option_id: optionId, user_id: user.id });
    if (error) {
      toast.error(error.message);
      return;
    }
    void qc.invalidateQueries({ queryKey: ["poll-results", announcementId] });
    void qc.invalidateQueries({ queryKey: ["my-poll-vote", announcementId, user.id] });
  };
      ...
      {options?.map((opt) => {
        ...
        return (
          <button
            ...
            className="relative w-full overflow-hidden rounded-lg border border-border p-2.5 text-left text-sm disabled:cursor-default"
          >
            {alreadyVoted && (
              <div
            ...
            <div className="relative flex items-center justify-between">
              <span className={isMine ? "font-semibold" : ""}>{opt.label}</span>
              {alreadyVoted && (
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {isLoading ? (
          ...
          <div className="space-y-5">
            {announcements.map((a) => (
              <article key={a.id} className="rounded-2xl bg-surface p-5">
                <div className="flex items-center gap-3">
                  <ChannelAvatar
                    path={a.author?.avatar_path ?? null}
                    name={a.author?.display_name || "CoreNetwork"}
                    size={36}
                  />
                  <div>
                    <p className="text-sm font-medium">
                      {a.author?.display_name || "Equipo CoreNetwork"}
                    </p>
                ...
                {a.post_type === "image" && a.image_path && (
                  <div className="mt-3 overflow-hidden rounded-xl bg-background">
                    <SignedImage path={a.image_path} alt={a.title} className="w-full object-cover" />
                  </div>
                )}

                {a.post_type === "poll" && <PollBlock announcementId={a.id} />}
              </article>
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border py-24 text-center">
            ...
            <p className="text-lg font-medium">Todavía no hay anuncios</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Aquí verás las novedades oficiales de CoreNetwork.
            </p>
          </div>
        )}
      </div>
