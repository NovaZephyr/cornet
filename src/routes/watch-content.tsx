...
type VideoRow = { id: string; code: string; user_id: string; title: string; description: string; video_path: string; thumbnail_path: string | null; views: number; created_at: string; category: string | null; age_restricted?: boolean };
...
export function WatchContent({ initialVideo }: { initialVideo: InitialWatchData }) {
  ...
  const videoQuery = useQuery({
    ...
    queryFn: async () => {
      const { data: video, error } = await supabase.from("videos").select("id, code, user_id, title, description, video_path, thumbnail_path, views, created_at, category, age_restricted").eq("code", code).eq("visibility", "public").maybeSingle();
  ...
  const shareVideo = async () => { if (!data?.video) return; const shareUrl = new URL(`/watch?v=${encodeURIComponent(data.video.code)}`, window.location.origin).toString(); const shareData = { title: data.video.title, text: `Mira “${data.video.title}” en CoreNetwork`, url: shareUrl }; try { if (navigator.share) { await navigator.share(shareData); return; } } catch (error) { if (error instanceof DOMException && error.name === "AbortError") return; } try { await navigator.clipboard.writeText(shareUrl); toast.success("Enlace del video copiado"); } catch { window.prompt("Copia este enlace para compartir el video:", shareUrl); } };
