...
function SeriesPage() {
  const series = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("playlists").select("id,title,description,visibility,user_id,created_at").eq("kind", "series").eq("visibility", "public").order("updated_at", { ascending: false }).limit(60);
