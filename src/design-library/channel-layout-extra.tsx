...
function Identity({ data, size = 72 }: { data: ChannelLayoutProps["data"]; size?: number }) {
  return <div className="cn-extra-identity"><ChannelAvatar path={data.profile.avatar_path} name={data.profile.display_name || data.profile.username} size={size} /><div><h1>{data.profile.display_name || data.profile.username}<ChannelDistinctions className="ml-2 inline-flex" isVerified={data.profile.is_verified} isMusic={data.profile.is_music_channel} roles={data.roles} /></h1><p>@{data.profile.username} · {data.profile.subscriber_count ?? data.subscriptions.length} suscriptores</p></div></div>;
}
