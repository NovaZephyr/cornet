...
function client() { if (!endpoint || !accessKeyId || !secretAccessKey || !bucket) throw new Error("B2 is not configured"); return new S3Client({ region, endpoint, forcePathStyle: true, credentials: { accessKeyId, secretAccessKey } }); }
...
function validKey(value: unknown, userId: string) {
  if (typeof value !== "string" || !value || value.startsWith("/") || value.includes("..") || value.length > 1024) throw new Error("Invalid object key");
...
function cleanFilename(value: unknown) { if (typeof value !== "string") return undefined; const cleaned = value.replace(/[\r\n"]/g, "").trim().slice(0, 180); return cleaned || undefined; }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "upload-url") {
      const contentType = typeof body?.contentType === "string" && body.contentType.length <= 255 ? body.contentType : "application/octet-stream";
      const expiresIn = Math.min(Math.max(Number(body?.expiresIn ?? 900), 60), 3600);
      const url = await getSignedUrl(s3, new PutObjectCommand({ Bucket: bucket!, Key: key, ContentType: contentType }), { expiresIn });
    ...
    if (action === "download-url") {
      ...
      if (supabaseUrl && serviceRoleKey) {
        ...
        const { data: record } = await admin.from("community_files").select("share_token,is_public").eq("user_id", user.id).eq("storage_path", key).maybeSingle();
        if (record?.share_token && record.is_public) {
          return json({ url: `${supabaseUrl}/functions/v1/b2-public-download?token=${encodeURIComponent(record.share_token)}`, key, expiresIn: 0, stable: true });
        }
      }
      const filename = cleanFilename(body?.filename);
      const head = await s3.send(new HeadObjectCommand({ Bucket: bucket!, Key: key }));
      const command = new GetObjectCommand({ Bucket: bucket!, Key: key, ...(filename ? { ResponseContentDisposition: `attachment; filename=\"${filename}\"` } : {}), ...(head.ContentType ? { ResponseContentType: head.ContentType } : {}) });
      const url = await getSignedUrl(s3, command, { expiresIn });
      return json({ url, key, expiresIn, size: head.ContentLength ?? 0, contentType: head.ContentType ?? null, stable: false });
    }
