...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET" && resource === "health") return json({ ok: true, service: "corenetwork-api", version: 6, timestamp: new Date().toISOString() });
    ...
    if (resource === "data" && req.method === "POST") {
      ...
      if (op === "select") {
        ...
        for (const f of Array.isArray(body.filters) ? body.filters : []) {
          if (!f || typeof f.column !== "string") continue; const value = f.value; const operator = String(f.operator ?? "eq");
          switch (operator) {
        ...
        if (Array.isArray(body.order)) for (const item of body.order) if (item?.column) query = query.order(item.column, { ascending: item.ascending !== false });
      ...
      if (op !== "insert" && op !== "upsert" && op !== "update" && op !== "delete") return json({ error: "OPERATION_NOT_ALLOWED" }, 400);
      const user = await requireUser(sb); if (!user) return json({ error: "UNAUTHORIZED" }, 401);
      if (op === "insert") query = query.insert(body.values); else if (op === "upsert") query = query.upsert(body.values); else { for (const f of Array.isArray(body.filters) ? body.filters : []) if (f?.column) query = query.eq(f.column, f.value); if (op === "update") query = query.update(body.values); else query = query.delete(); }
      query = query.select(typeof body.select === "string" ? body.select : "*");
      if (body.singleMode === "maybeSingle") query = query.maybeSingle(); else if (body.singleMode === "single") query = query.single();
    ...
    if (resource === "rpc" && req.method === "POST") {
      const body = await req.json().catch(() => ({})); const fn = typeof body.fn === "string" ? body.fn : "";
      if (!RPC_ALLOWLIST.has(fn) && !PUBLIC_RPCS.has(fn)) return json({ error: "RPC_NOT_ALLOWED" }, 403);
      const user = await requireUser(sb); if (!user && !PUBLIC_RPCS.has(fn)) return json({ error: "UNAUTHORIZED" }, 401);
      const { data, error } = await sb.rpc(fn, body.args ?? {}); return json({ data: data ?? null, error: error ? { message: error.message, code: error.code } : null });
    }
